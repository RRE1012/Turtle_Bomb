...
public class Bomb : MonoBehaviour {
    ...
    bool m_is_Blocked_N_init = false;
    bool m_is_Blocked_S_init = false;
    bool m_is_Blocked_W_init = false;
    bool m_is_Blocked_E_init = false;
    
    int m_FlameLength_index_N;
    int m_FlameLength_index_S;
    int m_FlameLength_index_W;
    int m_FlameLength_index_E;
    ...
    private void OnDestroy()
    {
        ...
        m_FlameLength_index_N = m_FlameCount;
            m_FlameLength_index_S = m_FlameCount;
            m_FlameLength_index_W = m_FlameCount;
            m_FlameLength_index_E = m_FlameCount;
        ...
        for (int i = 0; i < m_FlameCount; ++i)
        {
            ...
            // 길이 설정
            if (!m_is_Blocked_N_init && m_Blocked_N)
            {
                Debug.Log("North");
                m_FlameLength_index_N = i;
                m_is_Blocked_N_init = true;
                Debug.Log(m_FlameLength_index_N);
                Debug.Log(m_is_Blocked_N_init);
            }

            if (!m_is_Blocked_S_init && m_Blocked_S)
            {
                m_FlameLength_index_S = i;
                m_is_Blocked_S_init = true;
            }

            if (!m_is_Blocked_W_init && m_Blocked_W)
            {
                m_FlameLength_index_W = i;
                m_is_Blocked_W_init = true;
            }

            if (!m_is_Blocked_E_init && m_Blocked_E)
            {
                m_FlameLength_index_E = i;
                m_is_Blocked_E_init = true;
            }
        }


        Instance_FlameDir_N = Instantiate(m_Flame);
        Instance_FlameDir_N.transform.position = new Vector3(gameObject.transform.position.x, 0.0f, gameObject.transform.position.z + 2.0f);
        Instance_FlameDir_N.transform.localEulerAngles = new Vector3(0.0f, 90.0f, 0.0f);
        Instance_FlameDir_N.transform.localScale += new Vector3((2.0f * m_FlameLength_index_N), 0.0f, 0.0f);

        Instance_FlameDir_S = Instantiate(m_Flame);
        Instance_FlameDir_S.transform.position = new Vector3(gameObject.transform.position.x, 0.0f, gameObject.transform.position.z - 2.0f);
        Instance_FlameDir_N.transform.localEulerAngles = new Vector3(0.0f, -90.0f, 0.0f);
        Instance_FlameDir_S.transform.localScale += new Vector3((2.0f * m_FlameLength_index_S), 0.0f, 0.0f);

        Instance_FlameDir_W = Instantiate(m_Flame);
        Instance_FlameDir_W.transform.position = new Vector3(gameObject.transform.position.x - 2.0f, 0.0f, gameObject.transform.position.z);
        Instance_FlameDir_W.transform.localScale += new Vector3((2.0f * m_FlameLength_index_W), 0.0f, 0.0f);

        Instance_FlameDir_E = Instantiate(m_Flame);
        Instance_FlameDir_E.transform.position = new Vector3(gameObject.transform.position.x + 2.0f, 0.0f, gameObject.transform.position.z);
        Instance_FlameDir_N.transform.localEulerAngles = new Vector3(0.0f, 180.0f, 0.0f);
        Instance_FlameDir_E.transform.localScale += new Vector3((2.0f * m_FlameLength_index_E), 0.0f, 0.0f);
