...
public class NetUser2 : MonoBehaviour
{
    ...
    void IDCheck()
    {
        if (p != null)
        {
            if (Turtle_Move.instance.GetId() == 1 || Turtle_Move.instance.GetId() > 3)
            {
    ...
    void Update()
    {
        if (NetTest.instance.GetNetAlive(1) == 2)
        {
            glider_on = true;
            m_animator.SetBool("TurtleMan_GetGlider", true);

        }
        else if (NetTest.instance.GetNetAlive(1) == 1)
        {
            if (glider_on)
            {
                m_animator.SetBool("TurtleMan_GetGlider", false);
        ...
        if (VariableManager.instance.people_inRoom[1] == 0)
            p.SetActive(false);
        else if (VariableManager.instance.pos_inRoom - 1 == 1)
        {
            p.SetActive(false);
        }
        else
        ...
        SetPos(NetTest.instance.GetNetPosx(1), NetTest.instance.GetNetRoty(1), NetTest.instance.GetNetPosz(1));
