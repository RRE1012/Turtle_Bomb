...
#define SC_REMOVE_PLAYER 29
#define CASE_AIRDROP 30
...
#define SIZEOF_TB_KickBombRE 21
...
#define MAP_ENEMY 8
#define MAP_BOSS 9
#define MAP_GLIDERITEM 22
#define MAP_AIRDROPITEM 23


#define MAP_ICEBOX1 28
#define MAP_ICEBOX2 29
#define MAP_ICEBOX3 30
...
struct TB_DisConnect {
	unsigned char size;
	unsigned char type;//29
	short id;
};
struct TB_DBInfo_1 {
...
struct TB_RoomOut {//방에서 나갈 때
	...
	unsigned char roomID;
	unsigned char my_pos;
	unsigned char imwinner; //2일경우 lose
...
public:
	...
	Bomb_TB(int a, int b) {
		time = (float)GetTickCount() / 1000; explode_time = 3.0f;
		xz = make_pair(a, b);
		is_throw = false;
		is_kicked = false;
	}
...
public:
	...
	void SetAirMap() {
		for (int i = 0; i < 15; ++i) {
			for (int j = 0; j < 15; ++j) {
				if (map.mapInfo[i][j] == MAP_NOTHING) {
	...
	void SetTime(DWORD a) {
		...
		time = time - temp;
		airdrop_cooltime = airdrop_cooltime - temp;
	}

	bool OneSec() {
		if (((int)(time * 10) % 10)<1 &&time_check!=(int)(time)){
			time_check = (int)((int)time/1);
			return true;
		}
		else
			return false;
		...
		for (unsigned char b = 1; b <= f; ++b) {
			if (!l_DownBlock) {
				if (z - b < 0) {
					...
					if (tempMap[z - b][x] == MAP_BOMB) {
					...
					else if (tempMap[z - b][x] == MAP_BOX || tempMap[z - b][x] == MAP_ICEBOX1) {
						//cout << "Box!!!" << endl;
						int temp_rand = (rand() % 16);
						if (temp_rand < 4)
							tempMap[z - b][x] = MAP_NOTHING;
						...
						else if (temp_rand >= 14 && temp_rand <= 15)
							tempMap[z - b][x] = MAP_GLIDERITEM;

						l_DownBlock = true;
						df = b - 1;
					}
					else if (tempMap[z - b][x] == MAP_ICEBOX2) {
						tempMap[z - b][x] = MAP_ICEBOX1;
						l_DownBlock = true;
						df = b - 1;
					}
					else if (tempMap[z - b][x] == MAP_ICEBOX3) {
						tempMap[z - b][x] = MAP_ICEBOX2;
						l_DownBlock = true;
						df = b - 1;
					}
					else if (tempMap[z - b][x] == MAP_ITEM || tempMap[z - b][x] == MAP_GLIDERITEM || tempMap[z - b][x] == MAP_AIRDROPITEM|| tempMap[z - b][x] == MAP_ITEM_F || tempMap[z - b][x] == MAP_ITEM_S || tempMap[z - b][x] == MAP_KICKITEM || tempMap[z - b][x] == MAP_THROWITEM) {
						tempMap[z - b][x] = MAP_NOTHING;
					}
			...
			if (!l_LeftBlock) {
				if (x - b < 0) {
					...
					else if (tempMap[z][x - b] == MAP_BOX || tempMap[z][x - b] == MAP_ICEBOX1) {
						cout << "Box!!!" << endl;
						int temp_rand = (rand() % 16);
						if (temp_rand<4)
							tempMap[z][x - b] = MAP_NOTHING;
							...
							tempMap[z][x - b] = MAP_GLIDERITEM;
						l_LeftBlock = true;
						lf = b - 1;
					}
					else if (tempMap[z][x - b] == MAP_ICEBOX2) {
						tempMap[z][x - b] = MAP_ICEBOX1;
						l_LeftBlock = true;
						lf = b - 1;
					}
					else if (tempMap[z][x - b] == MAP_ICEBOX3) {
						tempMap[z][x - b] = MAP_ICEBOX2;
						l_LeftBlock = true;
						lf = b - 1;
					}
					else if (tempMap[z][x - b] == MAP_ITEM || tempMap[z ][x-b] == MAP_GLIDERITEM || tempMap[z ][x- b] == MAP_AIRDROPITEM || tempMap[z][x - b] == MAP_KICKITEM || tempMap[z][x - b] == MAP_THROWITEM || tempMap[z][x - b] == MAP_ITEM_F || tempMap[z][x - b] == MAP_ITEM_S) {
						tempMap[z][x - b] = MAP_NOTHING;
					}
...
public:
	...
	int exp;
	int exp_max;
	float communicate_time; //최근 교류한 시간
	...
	void ChangeRecieveTime() {
		communicate_time = GetTickCount();
	}

	bool IsUnConnected(DWORD t_time) {

		return ((float)t_time-communicate_time)/ 1000.0f >=20.0f;

	}
	bool IsUnConnected_inRoom(DWORD t_time) {

		return ((float)t_time - communicate_time) / 1000.0f >= 80.0f;

	}
	bool IsUnConnected_inGame(DWORD t_time) {

		return ((float)t_time - communicate_time) / 1000.0f >= 100.0f;
