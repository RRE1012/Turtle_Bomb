...
// 게임 씬 UI 함수
public class UI_Beta : MonoBehaviour {

    static UI_Beta m_Instance; public static UI_Beta GetInstance() { return m_Instance; }
    ...
    {
        m_Instance = this;
        StageManager_Copy.GetInstance().Init_Left_Time();
    ...
    void Start()
    {
        ...
        StageManager_Copy.GetInstance().GetQuestList(ref m_QuestList);
    ...
    IEnumerator Wait_For_Intro()
    {
        while (true)
        {
            if (StageManager_Copy.GetInstance().Get_is_Intro_Over())
            {
                StopAllCoroutines();
    ...
    {
        StageManager_Copy.GetInstance().Set_is_Pause(true);
    ...
    {
        if (!StageManager_Copy.GetInstance().Get_is_Pause())
        {
    ...
    void Mission_UI_Management()
    {
        for (int i = 0; i < 3; ++i)
        {
            ...
            {
                m_MissionText[i].text = m_QuestList[i].Quest_Script + " " + StageManager_Copy.GetInstance().Get_Left_Normal_Monster_Count().ToString() + " / " + m_monsterKill.ToString();
                if (StageManager_Copy.GetInstance().Get_Left_Normal_Monster_Count() >= m_monsterKill)
                    m_Mission_Star_Images[i].texture = m_Activated_Star_Texture;
            }
    ...
    IEnumerator UI_Update()
    {
        while(true)
        {
            if (!StageManager_Copy.GetInstance().Get_is_Pause())
            {
                ...
                StageManager_Copy.GetInstance().Summon_SuddenDeath_Glider();
                ...
                if (time_Second <= 0)
                {
                    time_Second = 0;
                    GameOver_Directing();
                    StageManager_Copy.GetInstance().Set_Game_Over();
                }
            }
            yield return null;
        }
    ...
    {
        StageManager_Copy.GetInstance().Set_is_Pause(false);
        m_Option_UI.SetActive(false);
    }
